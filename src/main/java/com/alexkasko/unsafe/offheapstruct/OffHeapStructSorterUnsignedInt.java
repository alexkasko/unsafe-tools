 ...
 * and adapted to {@link OffHeapStructCollection} with unsigned int keys.
 *
 ...
 */
class OffHeapStructSorterUnsignedInt {
    ...
    private static void doSort(OffHeapStructCollection a, long left, long right, int keyOffset, byte[] pi, byte[] pj,
        ...
        if (right - left + 1 < INSERTION_SORT_THRESHOLD) {
            for (long i = left + 1; i <= right; i++) {
                long ai = a.getUnsignedInt(i, keyOffset);
                a.get(i, pi);
                long j;
                for (j = i - 1; j >= left && ai < a.getUnsignedInt(j, keyOffset); j--) {
                    a.get(j, pj);
                    a.set(j + 1, pj);
                }
                a.set(j + 1, pi);
    ...
    private static void dualPivotQuicksort(OffHeapStructCollection a, long left, long right, int keyOffset, byte[] pi, byte[] pj,
        ...
        long ae1 = a.getUnsignedInt(e1, keyOffset), ae2 = a.getUnsignedInt(e2, keyOffset), ae3 = a.getUnsignedInt(e3, keyOffset),
                ae4 = a.getUnsignedInt(e4, keyOffset), ae5 = a.getUnsignedInt(e5, keyOffset);
        ...
        if (pivotsDiffer) {
            ...
            for (long k = less; k <= great; k++) {
                long ak = a.getUnsignedInt(k, keyOffset);
                a.get(k, pe1);
                if (ak < pivot1) { // Move a[k] to left part
                    ...
                    while (a.getUnsignedInt(great, keyOffset) > pivot2) {
                        if (great-- == k) {
                            break outer;
                        }
                    }
                    if (a.getUnsignedInt(great, keyOffset) < pivot1) {
                        a.get(less, pe3);
                        a.set(k, pe3);
                        a.get(great, pe3);
                        a.set(less++, pe3);
            ...
            for (long k = less; k <= great; k++) {
                ...
                if (ak < pivot1) { // Move a[k] to left part
                     ...
                     */
                    while (a.getUnsignedInt(great, keyOffset) > pivot1) {
                        great--;
                    }
                    if (a.getUnsignedInt(great, keyOffset) < pivot1) {
                        a.get(less, pe3);
         ...
         */
        if (less < e1 && great > e5) {
            while (a.getUnsignedInt(less, keyOffset) == pivot1) {
                less++;
            }
            while (a.getUnsignedInt(great, keyOffset) == pivot2) {
                great--;
