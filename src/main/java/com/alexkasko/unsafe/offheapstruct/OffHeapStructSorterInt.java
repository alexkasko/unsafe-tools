 ...
 * and adapted to {@link OffHeapStructCollection} with signed int keys.
 *
...
class OffHeapStructSorterInt {
    ...
    /**
     * Sorts the specified range of the off-heap struct collection into ascending order using signed int struct key. The range
    ...
    private static class MergeIter implements OffHeapDisposableIterator<byte[]> {
        ...
        public byte[] next() {
            ...
            for (int i = 0; i < indices.length; i++) {
                ...
                if (col.getInt(indices[minind], keyOffset) > col.getInt(indices[i], keyOffset)) minind = i;
            }
