 ...
 * and adapted to {@link OffHeapStructCollection} with unsigned byte keys.
 *
 ...
 */
class OffHeapStructSorterUnsignedByte {

    /**
     * Sorts the specified off-heap struct collection into ascending order using unsigned byte struct key.
     *
    ...
    /**
     * Sorts the specified range of the off-heap struct collection into ascending order using unsigned byte struct key.
    ...
    private static void doSort(OffHeapStructCollection a, long left, long right, int keyOffset, byte[] pi, byte[] pj,
        ...
        if (right - left + 1 < INSERTION_SORT_THRESHOLD) {
            for (long i = left + 1; i <= right; i++) {
                long ai = a.getUnsignedByte(i, keyOffset);
                a.get(i, pi);
                long j;
                for (j = i - 1; j >= left && ai < a.getUnsignedByte(j, keyOffset); j--) {
                    a.get(j, pj);
                    a.set(j + 1, pj);
                }
                a.set(j + 1, pi);
     ...
     * Dual-Pivot Quicksort algorithm using unsigned byte struct key.
     *
     * @param a the off-heap header-payload collection to be sorted
     * @param left the index of the first element, inclusive, to be sorted
    ...
    private static void dualPivotQuicksort(OffHeapStructCollection a, long left, long right, int keyOffset, byte[] pi, byte[] pj,
        ...
        long ae1 = a.getUnsignedByte(e1, keyOffset), ae2 = a.getUnsignedByte(e2, keyOffset), ae3 = a.getUnsignedByte(e3, keyOffset),
                ae4 = a.getUnsignedByte(e4, keyOffset), ae5 = a.getUnsignedByte(e5, keyOffset);
        ...
        if (pivotsDiffer) {
            ...
            for (long k = less; k <= great; k++) {
                long ak = a.getUnsignedByte(k, keyOffset);
                a.get(k, pe1);
                if (ak < pivot1) { // Move a[k] to left part
                    ...
                    while (a.getUnsignedByte(great, keyOffset) > pivot2) {
                        if (great-- == k) {
                            break outer;
                        }
                    }
                    if (a.getUnsignedByte(great, keyOffset) < pivot1) {
                        a.get(less, pe3);
                        a.set(k, pe3);
                        a.get(great, pe3);
                        a.set(less++, pe3);
            ...
            for (long k = less; k <= great; k++) {
                ...
                if (ak < pivot1) { // Move a[k] to left part
                     ...
                     */
                    while (a.getUnsignedByte(great, keyOffset) > pivot1) {
                        great--;
                    }
                    if (a.getUnsignedByte(great, keyOffset) < pivot1) {
                        a.get(less, pe3);
         ...
         */
        if (less < e1 && great > e5) {
            while (a.getUnsignedByte(less, keyOffset) == pivot1) {
                less++;
            }
            while (a.getUnsignedByte(great, keyOffset) == pivot2) {
                great--;
